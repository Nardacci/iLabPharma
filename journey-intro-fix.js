(function(){
  ...
  window.renderJourneyIntro=function(){
    ...
    visual.className='home-orbit journey-intro-home-orbit';
    visual.setAttribute('aria-label','Ecossistema de módulos');
    visual.innerHTML=`<div class="orbit-glow"></div><div class="orbit-ring ring-outer"></div><div class="orbit-ring ring-middle"></div><div class="orbit-ring ring-inner"></div><div class="orbit-cross cross-v"></div><div class="orbit-cross cross-h"></div><div class="orbit-core"><img src="assets/ilabpharma-logo.svg" alt="iLabPharma"></div>${modules.map(m=>`<div class="orbit-module ${m.cls}"><div class="module-icon">${icons[m.icon]}</div><b class="module-label">${m.title}</b></div>`).join('')}`;
  };
})();