const journeyStages=[
  ...
  {n:2,title:'Fábrica',kicker:'CAPÍTULO 02',intro:'Com a estrutura organizacional definida, a próxima etapa é representar o ambiente onde a operação acontece.',context:'A fábrica reúne os recursos físicos e operacionais utilizados ao longo da jornada.',flow:['Almoxarifados','Salas / Setores','Equipamentos','Instrumentos','Linhas','Centros de Trabalho','Localizações'],items:['Almoxarifados','Setores / Salas','Equipamentos','Instrumentos','Linhas','Centro de trabalhos','Localizações'].map((name,i)=>({name,text:'Elemento apresentado no processo de produção da fábrica.',source:'Processo de Produção · Fábrica'}))},
  {n:3,title:'Preparação',kicker:'CAPÍTULO 03',intro:'Antes de produzir, precisamos saber se a estrutura está preparada.',context:'A etapa reúne as condições apresentadas no processo para preparar a operação antes da fabricação.',flow:['Equipamentos qualificados','Equipamentos limpos','Calibrações válidas','Manutenções em dia','Salas liberadas'],items:['Equipamentos qualificados','Equipamentos limpos','Calibrações válidas','Manutenções em dia','Salas liberadas'].map(name=>({name,text:'Condição apresentada no processo de produção.',source:'Processo de Produção · Preparação'}))},
  {n:4,title:'Medicamento',kicker:'CAPÍTULO 04',intro:'O que será produzido?',context:'A etapa organiza os elementos apresentados como base para o medicamento.',flow:['Fornecedores','Matérias-Primas','Materiais de Embalagem','Produto','Fórmula','Especificações','Métodos Analíticos'],items:['Fornecedores','Matérias-Primas','Materiais de Embalagens','Produtos','Fórmulas','Especificações','Métodos Analíticos'].map(name=>({name,text:'Elemento apresentado no processo de produção do medicamento.',source:'Processo de Produção · Medicamento'}))},
  {n:5,title:'Produção',kicker:'CAPÍTULO 05',intro:'Da preparação à fabricação.',context:'A jornada chega à execução da produção, passando pelo planejamento e pelas etapas operacionais descritas no processo.',flow:['Planejamento','Ordem de Produção','Separação','Pesagem','Mistura','Granulação','Compressão','Revestimento','Embalagem'],items:['Planejamento','Ordem de Produção','Separação de materiais','Pesagem','Mistura','Granulação','Compressão','Revestimento','Embalagem'].map(name=>({name,text:'Etapa operacional apresentada no processo de produção.',source:'Processo de Produção · Produção'}))},
  {n:6,title:'Controle da Qualidade',kicker:'CAPÍTULO 06',intro:'O produto precisa ser avaliado.',context:'O produto passa pelas atividades de amostragem, análises e decisão de qualidade antes de seguir para a Garantia da Qualidade.',flow:['Amostragem','Análises em Processo','Análises Físico-Químicas','Aprovação / Reprovação','Liberação para GQ'],items:['Amostragem','Análises em Processo','Análises Físico-Químicas','Aprovação / Reprovação','Liberação para GQ'].map(name=>({name,text:'Atividade apresentada no processo de produção.',source:'Processo de Produção · Controle da Qualidade'}))},
  {n:7,title:'Garantia da Qualidade',kicker:'CAPÍTULO 07',intro:'A qualidade agora entra na decisão final.',context:'A etapa apresenta revisão, investigação, decisões sobre o lote e fechamento da operação.',flow:['Gestão da Qualidade','Revisão Documental','Investigação de Desvio','Aprovação do Lote','Liberação do Lote','Fechamento da OP'],items:['Gestão da Qualidade','Revisão documental','Investigação de desvio','Aprovação do lote','Liberação do lote','Fechamento da OP','Auditorias e CAPA'].map(name=>({name,text:'Elemento apresentado no processo ou na documentação do módulo de Garantia da Qualidade.',source:'Processo de Produção · Garantia da Qualidade'}))},
  {n:8,title:'Produto Acabado',kicker:'CAPÍTULO 08',intro:'O produto chegou ao resultado.',context:'A jornada chega ao produto acabado, estoque, expedição e rastreabilidade.',flow:['Produto Acabado','Liberação do Lote','Estoque','Expedição','Cliente'],items:['Produto acabado','Liberação do lote','Estoque','Expedição','Cliente','Rastreabilidade','Auditorias e CAPA'].map(name=>({name,text:'Elemento apresentado no processo de produção.',source:'Processo de Produção · Produto Acabado'}))}
];
let journeyIndex=0;
