...
function renderJourney(index=journeyIndex,selected=0){
  ...
  app.innerHTML=`<section class="journey-shell">
    ...
    <div class="journey-context"><span class="section-kicker">CONTEXTO</span><h3>${journeyIndex===0?'Estrutura antes da operação':'O ambiente onde a operação acontece'}</h3><p>${s.context}</p>${s.contextNote?`<p class="context-note">${s.contextNote}</p>`:''}</div>
