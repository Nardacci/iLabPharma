(function(){
  ...
  function renderIndustryFlowList(){
    ...
    app.innerHTML=`<section class="principal-flow-v4-screen">${flowHeader('Fluxos da Indústria','Processos representados para a etapa de Indústria.')}<div class="principal-flow-v4-layout">${sideBar()}<main class="principal-flow-v4-main principal-flow-list-main"><div class="principal-flow-v4-title"><span><i></i> FLUXOS DA INDÚSTRIA</span></div><div class="principal-flow-list"><button class="principal-flow-list-card" onclick="window.location.href='fluxo-modulo-principal.html'"><span class="principal-flow-list-number">01</span><div><strong>Módulo Principal</strong><p>Como os módulos utilizam os cadastros do Módulo Principal.</p></div><em>›</em></button><button class="principal-flow-list-card" onclick="renderIndustryArchitectureFlow()"><span class="principal-flow-list-number">02</span><div><strong>Arquitetura funcional</strong><p>Visão funcional do Módulo Principal e sua relação com os demais módulos.</p></div><em>›</em></button></div><div class="principal-flow-v4-bottom"><button class="flow-v4-ghost" onclick="renderJourney(0)">← Voltar para Indústria</button><div><span>VOCÊ ESTÁ AQUI</span><strong>01 · Indústria · Fluxos</strong></div><button class="flow-v4-primary" onclick="renderJourney(1)">Continuar →</button></div></main></div></section>`;
  }

  function renderIndustryPrincipalFlow(){
    window.location.href='fluxo-modulo-principal.html';
  }

  function renderIndustryArchitectureFlow(){
    setFlowMode();
    setCrumbs('Jornada','Indústria · Arquitetura funcional');
    app.innerHTML=`<section class="principal-flow-v4-screen" style="background:#fff;color:#102b4e"><header class="principal-flow-v4-header" style="border-bottom:1px solid #d9e1ea;background:#061a32"><button class="principal-flow-v4-brand" onclick="renderHome()" aria-label="Voltar para Home"><img src="assets/ilabpharma-logo.svg" alt="iLabPharma"></button><div class="principal-flow-v4-header-divider"></div><div class="principal-flow-v4-header-title"><strong>Arquitetura funcional</strong><span>Visão funcional do Módulo Principal e sua relação com os demais módulos.</span></div><div class="principal-flow-v4-actions"><button onclick="renderHome()" aria-label="Voltar para Home" title="Voltar para Home">⌂</button></div></header><button class="principal-flow-close" style="top:82px;right:34px;background:#061a32" onclick="renderIndustryFlowList()" aria-label="Fechar fluxo" title="Fechar fluxo">×</button><div style="position:absolute;inset:70px 0 0;display:flex;align-items:center;justify-content:center;padding:8px 24px 18px;box-sizing:border-box"><img src="assets/arquitetura-funcional.svg?v=3.0.0" alt="Arquitetura funcional do iLabPharma" style="width:100%;height:100%;object-fit:contain;display:block"></div></section>`;
  }
